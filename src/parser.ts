...
import { compoundOperators, operators, queryValueTypes } from './operators.js'
import type { ParsedNode, QueryInput } from './types.js'

const compoundKeys = R.keys(compoundOperators)
const operatorKeys = R.keys(operators)
const regexKey = '$regex'
const regexOptionsKey = '$options'

const isCompound = R.contains(R.__, compoundKeys)
const isOperator = R.contains(R.__, operatorKeys)
...
function throwOnInvalidQueryVal(q: { op: string; val: unknown }) {
  throw new Error(`${q.op} query value (${String(q.val)}) isn't valid`)
}

const assertValidQueryValue = R.compose(
  R.unless(
    R.converge(R.call, [R.compose(getQueryTypeFn, R.prop('op')), R.prop('val')]),
...
const formatElemMatch = R.curryN(
  2,
  R.applySpec({
...
const getRest = R.converge(R.objOf, [
  R.head,
  R.compose(R.last, R.head, R.toPairs, R.last),
])

const getKey = R.compose(R.head, R.head, R.toPairs, R.last)

const switchValues = R.converge(R.pair, [getKey, getRest])
...
const formatWithOperatorPair = R.compose(
  formatQuery,
  R.unnest,
  R.ifElse(R.path([1, regexKey]), R.adjust(1, regexPair), R.adjust(1, singlePair))
)
...
const mergeSplitOperatorQueries = R.converge(R.map, [
  R.compose(R.prepend, R.head),
  splitMultilpleOperatorQueries,
])

function parsePair(pair: [string, unknown]): ParsedNode {
  return R.cond([
    [
      headIsCompoundWithMultiple,
      R.converge(formatCompound, [
        R.head,
        R.compose(
    ...
    [
      ...
      R.converge(formatCompound, [
        R.head,
        R.compose(R.map(parsePair), R.unnest, lastToPairs),
      ]),
    ],
    ...
    [
      explicitMultipleOperators,
      R.compose(formatCompound('$and'), R.map(parsePair), mergeSplitOperatorQueries),
    ],
...
function parseQueryValue(thing: QueryInput): ParsedNode[] {
  return R.cond([
    [R.is(Array), R.map(parseQueryValue)],
    [R.is(Object), R.compose(R.map(parsePair), R.toPairs)],
  ])(thing) as ParsedNode[]
}

const isQueryLike = R.propSatisfies(R.contains(R.__, ['query', 'elemMatch']), '_type')
...
const parseQuery = R.compose(
  ...
  R.unnest,
  parseQueryValue
) as (query: QueryInput) => ParsedNode[]
