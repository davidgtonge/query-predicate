...
import parseQuery from '../dist/parser.js'
    ...
    [
      {
        _type: 'compound',
        op: '$and',
        queries: [{ _type: 'query', key: 'foo', op: '$equal', val: true }],
      },
    ],
  ],
  'explicit operator': [
        ...
        queries: [
          {
            _type: 'elemMatch',
            key: ['foo'],
            queries: [{ _type: 'query', key: 'bar', op: '$equal', val: true }],
          },
        ],
      },
    ],
  ...
  'multiple compound operators': [
    ...
    [
      ...
      },
      {
        _type: 'compound',
        op: '$not',
        queries: [{ _type: 'query', key: 'bar', op: '$equal', val: false }],
      },
    ],
  ],
}
...
describe('parser', () => {
  function createTest(spec, name) {
    it(`parses: ${name}`, () => {
      const result = parseQuery(spec[0])
